...
def select_zones(zone_or_region: str, preferred: tuple[str, ...] = PREFERRED_ZONES) -> list[str]:
    ...
    stockout fallback. The cross-region tail is the vendor-shipping
    default and matches the documented multi-week stockout playbook;
    operators that want to pin a single region can pass a full zone.
    """
...
def wait_for_public_ip(
    ...
    *,
    ...
    while True:
        ...
        except Exception as exc:  # pragma: no cover — transient describe
            print(f"Warning: instances.get transient error: {exc}", file=sys.stderr)
