...
target project. If any of these are missing, the stub returns honest
``skipped=true`` evidence with the original API error rather than
emitting fabricated PASS.
...
def _skipped(reason: str) -> dict[str, Any]:
    return {"passed": False, "skipped": True, "skip_reason": reason}
...
def _modify_iam_policy(
    ...
    *,
    ...
    for attempt in range(max_retries):
        get_status, policy = _rest("POST", f"{resource_url}:getIamPolicy", caller_token, body={})
...
def _run_pre_provisioned_path(
    ...
    if denied_token is None or allowed_token is None:
        return _skip_all(result, f"token minting failed (denied={derr} | allowed={aerr})")
...
def _skip_all(result: dict[str, Any], reason: str) -> int:
    ...
    a denied probe ship as a passing RBAC validation. Set explicit
    failure state and rc 1; ``main`` prints exactly one JSON document
    for the run, so this helper must NOT print.
    """
    for name in REQUIRED_TESTS:
        result["tests"][name] = _skipped(reason)
    result["skipped"] = True
...
def _run_self_provision_path(
    ...
    try:
        ...
        if denied_email is None:
            rc = _skip_all(result, f"create denied SA failed: {err}")
            return rc
        ...
        if allowed_email is None:
            rc = _skip_all(result, f"create allowed SA failed: {err}")
            return rc
        ...
        if "iam.gserviceaccount.com" in caller_email:
            ...
            rc = _skip_all(result, f"unresolvable caller identity: {caller_email!r}")
            return rc
        ...
        for sa_email in (denied_email, allowed_email):
            ...
            if err:
                rc = _skip_all(result, f"tokenCreator on {sa_email} failed: {err}")
                return rc
        ...
        if not ok:
            rc = _skip_all(result, f"create scoping VM failed: {vm_err}")
            return rc
        ...
        if binding_err:
            rc = _skip_all(result, f"target-VM IAM bind failed: {binding_err}")
            return rc
        iam_bindings_added.append((target_url, SCOPED_ROLE, f"serviceAccount:{allowed_email}"))

        # IAM propagation is eventually consistent; wait a few seconds
        # before probing serial output as the new principal.
        time.sleep(8)

        denied_token, derr = _mint_token_for_sa(denied_email, caller_token)
        allowed_token, aerr = _mint_token_for_sa(allowed_email, caller_token)
        if denied_token is None or allowed_token is None:
            rc = _skip_all(result, f"token minting failed (denied={derr} | allowed={aerr})")
            return rc
...
def main() -> int:
    ...
    if caller_token is None:
        rc = _skip_all(result, "ADC token refresh failed; cannot run RBAC probes")
    else:
        ...
        if denied_env and allowed_env and other_env:
            ...
            rc = _skip_all(result, "could not resolve calling identity; cannot self-provision RBAC probes")
        else:
