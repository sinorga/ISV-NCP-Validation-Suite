...
def main() -> int:
    ...
    try:
        ...
        # Operation completion is not the same as the instance being
        # observably stopped. Poll instances.get until canonical state is
        # ``stopped`` (GCE ``TERMINATED``) or the budget runs out — matches
        # the AWS oracle's ``instance_stopped`` waiter shape.
        observed_state = ""
        poll_deadline = time.monotonic() + 300
        while True:
            inst = client.get(project=project, zone=zone, instance=args.instance_id)
            observed_state = canonical_state(getattr(inst, "status", None))
            if observed_state == "stopped":
                break
            if time.monotonic() >= poll_deadline:
                break
            time.sleep(5)
        result["state"] = observed_state
        result["success"] = observed_state == "stopped"
        if not result["success"]:
            result["error"] = f"post-stop state is {observed_state}; expected stopped"
    except Exception as exc:
