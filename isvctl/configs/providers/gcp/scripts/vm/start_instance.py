...
def main() -> int:
    ...
    try:
        # 1. Pre-check current state. The AWS oracle and GCP knowledge
        # both require the canonical stopped state before issuing start;
        # a previously-running VM or a skipped/no-op stop step must not
        # produce a green start, otherwise the lifecycle test no longer
        # proves stop→start behavior (oracle parity rule #1).
