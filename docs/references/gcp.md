...
| `GCP_K8S_UNAUTHORIZED_PROBE_CMD` | Optional (k8s) | unset — `K8sApiNetworkAclCheck` structured-skips | Shell command that probes the cluster's Kubernetes API endpoint **from an outside vantage point that should be blocked** (e.g. an unauthorized-network reachability test). It is emitted into `K8sApiNetworkAclCheck.commands.unauthorized_probe`; when set, that check ENFORCES that the API refuses the blocked source. When **unset (the default)** the check has no outside vantage point and safely, honestly structured-skips — the override only ever ACTIVATES the probe, never weakens the check. GKE authorized-networks (`master_authorized_networks_config`) provides the capability. |
