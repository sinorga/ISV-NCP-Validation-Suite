...
| `GCP_OBSERVABILITY_SKIP_TEARDOWN` | Optional (observability) | unset — teardown runs | When `true`, the four observability teardown steps return success without deleting the run-created resources — the internal telemetry peer instance and the telemetry peer firewall rule (a source-scoped ICMP rule), the host instance, local SSH key pair, SSH firewall rule, created subnetworks, VPC Flow Logs configuration, and VPC network; forwarded as `--skip-destroy`. The same `--skip-destroy` flag also reaches the three setup producers (`create_network`, `launch_host`, `launch_telemetry_peer`), where it suppresses the compensating deletion a **setup failure** would otherwise run so a partially created run is preserved for inspection with its exact ownership identifiers. See the [Observability guide](../../isvctl/configs/providers/gcp/scripts/observability/docs/gcp-observability.md). |
